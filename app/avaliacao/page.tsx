...
type Tela = "menu" | "temas" | "prova" | "resultado";
...
export default function AvaliacaoPage() {
  ...
  const carregarTemas = useCallback(async () => {
    ...
    if (data) {
      const unicos = Array.from(new Set(data.map((d: { tema: string }) => d.tema))).sort() as string[];
      setTemas(unicos);
    }
  }, [supabase]);
  ...
  function responder(letra: string) {
    ...
    }, { onConflict: "user_id,questao_id" }).then();
  }

  function avancar() {
    if (indice + 1 >= questoes.length) setTela("resultado");
      ...
      <div className="flex flex-col gap-5 pb-4">
        ...
        <div className="flex flex-col gap-2">
          {q.alternativas.map(({ letra, texto }) => {
            ...
            if (respondeu) {
              if (letra === q.gabarito) estilo = "bg-green-100 border-green-500 text-green-800 font-semibold";
              else if (letra === respostaUser) estilo = "bg-red-100 border-red-400 text-red-800";
              else estilo = "bg-white/30 border-yas-ink/10 text-yas-ink/40";
            }
            return (
              <button
                key={letra}
        ...
        {/* justificativa — aparece após responder */}
        {respondeu && (
          <div className={`rounded-xl p-4 border ${acertou ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
            ...
            {q.explicacoes && Object.entries(q.explicacoes).length > 0 && (
              <div className="flex flex-col gap-1.5 border-t border-yas-ink/10 pt-3 mt-2">
                <p className="font-body text-[10px] font-semibold text-yas-ink/50 uppercase tracking-wide">Por que as outras estão erradas</p>
                {Object.entries(q.explicacoes).map(([l, exp]) => (
  ...
  // por tema (só na avaliação completa)
